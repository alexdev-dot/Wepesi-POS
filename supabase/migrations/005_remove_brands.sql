...
as $$
  ...
  begin
    insert into public.products (
      ...
      product_business_id, product_name, nullif(product_description, ''),
      coalesce(nullif(product_sku, ''), 'ITEM-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 12))),
      nullif(product_barcode, ''), product_category, product_cost_price,
      product_selling_price, product_reorder_level, greatest(opening_quantity, 0),
      product_image_url
