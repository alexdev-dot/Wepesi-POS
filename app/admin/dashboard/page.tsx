...
import { Building2, Users, CreditCard, Receipt, TrendingUp, AlertCircle, CheckCircle, Clock, Activity } from "lucide-react"
    ...
    <div className="space-y-6">
            ...
            {recentActivity.map((activity) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500">{activity.tenant}</p>
                </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <button className="flex items-center gap-3 p-4 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all">
            <Building2 className="h-5 w-5 text-blue-600" />
            <span className="text-sm font-medium text-slate-800">Add Tenant</span>
          </button>
          <button className="flex items-center gap-3 p-4 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all">
            <Users className="h-5 w-5 text-purple-600" />
            <span className="text-sm font-medium text-slate-800">Add Admin User</span>
          </button>
          <button className="flex items-center gap-3 p-4 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all">
            <CreditCard className="h-5 w-5 text-green-600" />
            <span className="text-sm font-medium text-slate-800">Manage Plans</span>
          </button>
          <button className="flex items-center gap-3 p-4 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all">
            <Receipt className="h-5 w-5 text-amber-600" />
            <span className="text-sm font-medium text-slate-800">View Invoices</span>
          </button>
        </div>
      </div>
    </div>
